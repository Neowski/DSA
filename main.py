...
def show_card():
    ...
    if current_cards:
        ...
        clear_flashcard_display()
# Function to reveal the current card and display its definition
def reveal_card():
    global card_index
    global current_cards
    if current_cards:
...
if __name__ == '__main__':
    ...
    ttk.Label(create_set_frame, text='Language:').pack(padx=5, pady=5)
    ...
    add_word_button = ttk.Button(create_set_frame, text='Add Word', command=add_word)
    add_word_button.pack(side='bottom', pady=50)
    add_word_button.config(padding=(0,1))

    
    # Create the "Select Set" tab and its content
    select_set_frame = ttk.Frame(notebook)
    notebook.add(select_set_frame, text="Saved Set")
    ...
    ttk.Button(select_set_frame, text='Choose', command=select_set).pack(padx=5, pady=5)
    # Button to delete a set 
    ttk.Button(select_set_frame, text='Delete', command=delete_selected_set).pack(padx=5, pady=5)
    ...
    # Button to reveal the flashcard 
    ttk.Button(flashcards_frame, text='reveal', command=reveal_card).pack(side='left', padx=5, pady=5)
